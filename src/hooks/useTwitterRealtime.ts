...
export function useTwitterRealtime(options?: { url?: string; mode?: 'demo' | 'poll'; userId?: string; bearer?: string }) {
  const { url = '/api/realtime/twitter?mode=demo', mode, userId, bearer } = options || {};
  ...
  useEffect(() => {
    ...
    const connectUrl = new URL(url, typeof window !== 'undefined' ? window.location.origin : '');
