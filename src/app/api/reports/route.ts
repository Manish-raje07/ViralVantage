...
export async function GET(request: NextRequest) {
  try {
    ...
    let posts = await postsCollection.find(query).sort({ publishedAt: -1 }).toArray();
    ...
    // Get insights
    const insightsCollection = await getCollection<Insight>(COLLECTIONS.INSIGHTS);
    const insights = await insightsCollection.find({}).sort({ generatedAt: -1 }).limit(10).toArray();
...
function buildReportData(posts: SocialPost[], insights: Insight[]) {
  ...
  const summary = {
    ...
    avgEngagementRate: posts.length > 0 
      ? Math.round((posts.reduce((sum, p) => sum + p.metrics.engagementRate, 0) / posts.length) * 100) / 100
      : 0,
  };
  
  // Platform breakdown
  const platformBreakdown: Record<string, { posts: number; likes: number; comments: number; shares: number; engagement: number }> = {};
  for (const post of posts) {
    if (!platformBreakdown[post.platform]) {
      platformBreakdown[post.platform] = { posts: 0, likes: 0, comments: 0, shares: 0, engagement: 0 };
    }
  ...
  const typeBreakdown: Record<string, { posts: number; avgEngagement: number; totalLikes: number }> = {};
  for (const post of posts) {
  ...
  for (const type in typeBreakdown) {
    typeBreakdown[type].avgEngagement = Math.round((typeBreakdown[type].avgEngagement / typeBreakdown[type].posts) * 100) / 100;
  }
  ...
  // Daily performance
  const dailyPerformance = new Map<string, { posts: number; engagement: number; reach: number }>();
  for (const post of posts) {
    const dateStr = new Date(post.publishedAt).toISOString().split('T')[0];
    const current = dailyPerformance.get(dateStr) || { posts: 0, engagement: 0, reach: 0 };
    current.posts++;
...
function generateCSVResponse(reportData: ReturnType<typeof buildReportData>) {
  ...
  // Header
  rows.push('Platform,Type,Content,Likes,Comments,Shares,Reach,Impressions,Engagement Rate,Published At,Hashtags');
