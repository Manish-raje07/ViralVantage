...
export async function GET(request: NextRequest) {
  try {
    ...
    if (type === 'overview') {
      ...
      const platforms = await db.collection<PlatformMetrics>('platforms')
        .find(platformFilter)
        .toArray();
      ...
      const avgEngagement = platforms.length > 0 
        ...
        : 0;
      const avgGrowthRate = platforms.length > 0
        ? platforms.reduce((sum, p) => sum + p.growthRate, 0) / platforms.length
        : 0;
      ...
      const dailyMetrics = await db.collection<DailyMetric>('dailyMetrics')
      ...
      return NextResponse.json({
        ...
        data: {
          overview: {
            ...
            roi: calculateROI(totalReach, totalLikes + totalComments + totalShares),
          },
    ...
    if (type === 'posts') {
      ...
      const total = await db.collection('posts').countDocuments(platformFilter);
    ...
    if (type === 'audience') {
      const audienceData = await db.collection<AudienceData>('audience')
        .find(platformFilter)
        .toArray();
...
function aggregateDailyMetrics(metrics: DailyMetric[], days: number) {
  ...
  metrics.forEach(m => {
    ...
    if (!groupedByDate[dateKey]) {
      groupedByDate[dateKey] = { followers: 0, engagement: 0, reach: 0, impressions: 0, posts: 0, count: 0 };
    }
  ...
  const filteredLabels = labels.filter((_, i) => i % interval === 0 || i === labels.length - 1);

  return {
    labels: filteredLabels.map(d => formatDateLabel(d, days)),
    followers: {
      data: filteredLabels.map(d => groupedByDate[d]?.followers || 0),
      growth: calculateGrowth(filteredLabels.map(d => groupedByDate[d]?.followers || 0)),
    },
    engagement: {
      data: filteredLabels.map(d => +(groupedByDate[d]?.engagement / (groupedByDate[d]?.count || 1)).toFixed(2) || 0),
    },
    reach: {
  ...
  return {
    demographics: {
      age: Object.entries(ageGroups).map(([range, values]) => ({
        range,
        percentage: Math.round(values.reduce((a, b) => a + b, 0) / values.length),
      })),
